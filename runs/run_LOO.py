...
def process_file(input_file, seq_dict):
    ...
    output_file = f'./LOO/scores_LOO_{pdb_id}.csv'  # Output file
    ...
    if reference_allele and isinstance(reference_allele, str):
        command = [
            ...
            '-p', './pdb_files/',
            '-t', f'./input/input_nr/{input_file}',  # Use the current input file
            '-e', f'{pep_seq}',  # Peptide sequence
            '-m', './input/input_MHCs.csv',
            '-a', f'{reference_allele}',  # Correct formatting for allele
            '-pp', f'./model/LOO_models/TCRen_TCR_p_LOO_{pdb_id}.csv',
            '-mhcp', f'./model/LOO_models/TCRen_TCR_MHC_LOO_{pdb_id}.csv',
...
def main():
    ...
    input_files = [f for f in os.listdir('./input/input_nr') if f.endswith('.csv')]
