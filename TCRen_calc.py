...
def calculate_TCRen(df, peptide=False):
    """
    Calculate TCRen potential for all residue pairs based on observed and expected frequencies.
    ...
    # Calculate TCRen potential
    contact_counts['TCRen'] = -np.log(contact_counts['pobs'] / contact_counts['pexp'])
    
    return contact_counts[['residue_from', 'residue_to', 'TCRen']]
    
def get_TCRen(row, df_residues, col1_id, col2_id):
    """
    Retrieve the TCRen potential value for a given residue pair from a DataFrame.
    
    Args:
        row (pd.Series): A row from the DataFrame containing contact information.
        df_residues (pd.DataFrame): DataFrame containing TCRen potential values.
        col_id (str): The column name that identifies the residue in the row.
    
    Returns:
        float or None: The TCRen potential value if a match is found; otherwise, None.
    """
    try:
