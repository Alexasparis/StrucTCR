...
def process_file(input_file, seq_dict):
    ...
    output_file = f'./test_set_similar/test_similar_{pdb_id}_all.csv'  # Output file
    ...
    if reference_allele and isinstance(reference_allele, str):
        command = [
            ...
            '-p', './pdb_files/',
            '-t', f'./input/input_test_similar/{input_file}',  # Use the current input file
...
def main():
    ...
    input_files = [f for f in os.listdir('./input/input_test_similar') if f.endswith('.csv')]
