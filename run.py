...
def process_file(input_file, seq_dict):
    ...
    output_file = f'scores_training_{pdb_id}.csv'  # Output file
    ...
    if reference_allele and isinstance(reference_allele, str):
        command = [
            'python3', 'main_fast2.py',
            '-p', './pdb_files/',
            '-t', f'./input/input_test/{input_file}',  # Use the current input file
            '-e', f'{pep_seq}',  # Peptide sequence
            '-m', './input/input_MHCs.csv',
            '-a', f'{reference_allele}',  # Correct formatting for allele
            '-pp', './model/TCRen_TCR_p_train.csv',
            '-mhcp', './model/TCRen_TCR_MHC_train.csv',
...
def main():
    ...
    seq_dict = parse_general_file('../TCRranker/structures_annotation/general.txt')
    print("Initialized sequence dictionary from general.txt.")

    # List to store the input files to be processed
    input_files = [f for f in os.listdir('./input/input_test') if f.endswith('.csv')]
