...
# 1) extract_contacts (pdb_file_list, chain_dict, distance=5)
# 2) filter_contacts(contacts_df, tcra_chain_id, tcrb_chain_id, peptide_chain_id, mhc_chain_id, threshold_n_atoms, remove_X=True)
...
def extract_contacts(pdb_files, chain_dict, distance=5):
    ...
    # Ensure residue_mapping is defined
    if residue_mapping is None:
        print("Error: residue_mapping is None")
        return pd.DataFrame()  # Return empty DataFrame if residue_mapping is not found
