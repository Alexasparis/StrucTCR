...
def main():
        ...
        if pdb_id_similar:
            ...
            if not os.path.isfile(pdb_file_path):
                ...
                if all(chains.values()):  # Ensure all chain identifiers are present
                    contacts_TCR_p, _ = filter_contacts(
                        contacts_df,
                        chains['tcra_chain'],
                        chains['tcrb_chain'],
    ...
    if args.mhc_allele:
        ...
        print(f"\nSearching similar MHC contact map for {reference_allele}\n")
        ...
        for pdb_file in os.listdir("./pdb_files"):
            if pdb_file.endswith(".pdb"):
                ...
                pdb_file_path = os.path.join("./pdb_files", pdb_file)
                chains_mhc = chain_dict.get(pdb_id, {})
                seq_pdb = extract_sequences(pdb_file_path)
                # Perform global alignment between sequences
                aligned_seq_pdb, aligned_seq_query, score = global_alignment(mhc_seq,seq_pdb[chains_mhc['mhc_chain']])        
        ...
        similar_mhc = "['" + "', '".join(max_pdb_ids) + "']"

        # Print only the closest TCR in the desired format
        print(f"Closest MHC for {pdb_id}: {similar_mhc}")
        
        if isinstance(similar_mhc, str):
            if similar_mhc.startswith("[") and similar_mhc.endswith("]"):
                pdb_id_mhc = ast.literal_eval(similar_mhc)
            else:
                pdb_id_mhc = [similar_mhc]
        else:
            pdb_id_mhc = [similar_mhc]

        if len(pdb_id_mhc) > 1:
            distances_mhc = []
            for pdb in pdb_id_mhc:
                pdb_file_path_mhc = os.path.join("./pdb_files", f"{pdb}.pdb")  
                if os.path.isfile(pdb_file_path_mhc):            
                    a_seq, b_seq, e_seq_mhc = extract_specific_sequences(pdb_file_path_mhc, seq_dict)
                    distance_mhc = calculate_sequence_distance(epitope_seq, e_seq_mhc)
                    distances_mhc.append(distance_mhc)
                else:
                    print(f"File not found: {pdb_file_path_mhc}")

            if distances_mhc:
                closest_pdb_mhc_index = distances_mhc.index(min(distances_mhc))
                pdb_id_mhc = pdb_id_mhc[closest_pdb_mhc_index]
                print(f"Match found for MHC of {tcr_id}: {pdb_id_mhc}.")

        elif len(pdb_id_mhc) == 1:
            pdb_id_mhc = pdb_id_mhc[0]
            print(f"Match found for MHC of {tcr_id}: {pdb_id_mhc}.")
        else:
            print(f"No similar MHCs found for {tcr_id}.")
            
        # EXTRACT CONTACTS FROM PDB_FILE:

        print(f"\n ## Processing MHC allele: {reference_allele}")
            
        if pdb_id_mhc:
            pdb_file_path_mhc = os.path.join("./pdb_files", f"{pdb}.pdb")
            if not os.path.isfile(pdb_file_path_mhc):
                print(f"Error: The file does not exist at {pdb_file_path_mhc}")
            else:
                contacts_df = pd.read_csv(f"./contact_maps/{pdb_id_mhc}_contacts.csv")
                chains_mhc = chain_dict.get(pdb_id_mhc, {})
                if all(chains_mhc.values()):  # Ensure all chain identifiers are present
            ...
            # Mapping with IMGT numbering
            print("Renumbering sequences with IMGT convention")
            alpha_mhc, beta_mhc, epitope_mhc = extract_specific_sequences(pdb_file_path_mhc, seq_dict)

            anarci_A = run_anarci(alpha_mhc, 'alpha')
            anarci_B = run_anarci(beta_mhc, 'beta')
            parsed_anarci_A = parse_anarci_output(anarci_A)
            parsed_anarci_B = parse_anarci_output(anarci_B)

            residues_A = extract_residues_and_resids(pdb_file_path_mhc, chains_mhc['tcra_chain'])
            residues_B = extract_residues_and_resids(pdb_file_path_mhc, chains_mhc['tcrb_chain'])

            mapping_A = map_imgt_to_original(parsed_anarci_A, residues_A)
            mapping_B = map_imgt_to_original(parsed_anarci_B, residues_B)

            imgt_mappings_mhc = {pdb_id_mhc: {chains_mhc['tcra_chain']: mapping_A, chains_mhc['tcrb_chain']: mapping_B}}
            contacts_imgt_M = add_imgt_mappings(contacts_TCR_MHC, imgt_mappings_mhc)

            contacts_TCR_MHC[tcr_id] = contacts_TCR_MHC.apply(lambda row: imgt_dict_D.get(row['imgt_from'], None) if row['chain_from'] == chains_mhc['tcra_chain']
                        else imgt_dict_E.get(row['imgt_from'], None) if row['chain_from'] == chains_mhc['tcrb_chain']
                        else None, axis=1)
        ...
        for element in scores_tcr_p:
            mhc_alleles.append(reference_allele if 'reference_allele' in locals() else None)  
